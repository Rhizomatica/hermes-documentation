...
\textbf{Shared memory interface (control plane).} The controller exposes a System V shared memory segment (key \verb|66650|, type
\verb|controller_conn|, see \verb|hermes-net/include/sbitx_io.h|). This interface is used by \verb|sbitx_client| and also by \verb|uucpd|
when configured with \verb|-o shm|, allowing the modem/TNC integration to key PTT (and report status such as bitrate/SNR/bytes) without
direct serial/CAT access.

\textbf{Configuration files.} The default configuration files are expected at:
\begin{itemize}
...
\textbf{Service unit.} The reference systemd unit file shipped in \verb|hermes-net| is:
\begin{verbatim}
hermes-net/system_services/init/sbitx.service
  ExecStart=/usr/bin/sbitx_controller
\end{verbatim}

\textbf{Health checks.} Typical checks include:
\begin{verbatim}
