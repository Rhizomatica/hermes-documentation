...
\paragraph{x11.service + vnc.service (headless VARA).} In the reference Debian images, VARA HF is run under Wine and requires an X server. For
...
\paragraph{modem.service (systemd unit).} A reference unit file is shipped as \verb|modem.service| (see
...
\subsubsection{UUCP configuration (/etc/uucp)}
...
\subsubsection{uucpd.service (systemd unit)}
...
In the reference VARA deployment (Wine VARA + VNC), the \verb|uucpd.service| unit is:
\begin{verbatim}
After=vnc.service sbitx.service
ExecStart=/usr/bin/uucpd -a 127.0.0.1 -p 8300 -r vara -o shm -f 2750p -m
\end{verbatim}

In the open-modem (mercury) deployment, \verb|hermes-net| ships \verb|uucpd-mercury.service| and installs it as \verb|uucpd.service|:
\begin{verbatim}
...
\subsubsection{Running uucpd (examples)}
...
\subsubsection{Command-line usage (uucpd / uuport)}
\begin{verbatim}
