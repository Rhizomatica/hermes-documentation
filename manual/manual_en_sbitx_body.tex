...
The HERMES installation procedure will set up the sBitx radio with HERMES system,
...
The radio (sBitx) has a BNC female connector, and needs the appropriate cables and adapters to
...
\section{Further troubleshooting (maintainers)}

This installation guide focuses on hardware setup and basic functional tests. For a detailed HERMES system architecture and software stack
description (radio control via \texttt{sbitx\_controller}, modem/TNC integration, \texttt{uucpd}/\texttt{uuport}, and gateway scheduling),
see the document \textbf{``HERMES Software Description''} (\texttt{software\_en.pdf}) in the HERMES documentation repository.

Useful service-level commands:
\begin{verbatim}
systemctl status sbitx uucpd
