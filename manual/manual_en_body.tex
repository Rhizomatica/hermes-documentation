...
\linebreak
\linebreak
Chrome/Chromium is the recommended browser for using the HERMES interface. For the best experience, please use one of these browsers.
...
\pagebreak
\ifdefined\HERMESCOMBINED
\else
\appendix{techinfo}
\fi
\label{techinfo}
\section{Appendix - technical information}
\textit{Maintainers note:} this user manual focuses on station operation and the web interface.
For a detailed HERMES system architecture and software stack description (e.g. \texttt{uucpd}/\texttt{uuport}, \texttt{sbitx\_controller},
modem/TNC integration, and systemd service orchestration), see the document \textbf{``HERMES Software Description''} (\texttt{software\_en.pdf})
in the HERMES documentation repository.
...
\begin{itemize}
    ...
    \item UUCP-to-modem bridge (\texttt{uucpd} + \texttt{uuport}) for VARA-style TNC integration, written in C:
    \url{https://github.com/Rhizomatica/hermes-net/tree/main/uucpd}
\end{itemize}
